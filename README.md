...
ただし luit は NEC 特殊文字・IBM 拡張文字を正しく変換できない(`①` が `ⴡ` に、`№` が `ⵢ` になる)。luit が ISO 2022 で登録された文字集合(JIS X 0208 / JIS X 0212)しか持たないためで、波ダッシュと半角カナは正しく扱える。これらの文字を含むファイルを閲覧するだけなら解決法1の `nkf` や `lv` を使う
