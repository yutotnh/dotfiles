...
実務の EUC-JP ファイルには NEC 特殊文字(`①`、`№`)や IBM 拡張文字が頻出する。これらの扱いでコマンドの挙動が分かれる

| コマンド             | 波ダッシュ `0xA1C1` | NEC 特殊文字・IBM 拡張文字        |
| -------------------- | ------------------- | --------------------------------- |
| `iconv -f EUC-JP`    | U+301C `〜`         | `illegal input sequence` で撥ねる |
| `iconv -f EUC-JP-MS` | U+FF5E `～`         | 扱える                            |
| `nkf -E -w -x`       | U+301C `〜`         | 扱える                            |

`iconv -f EUC-JP` は `①` が 1 文字混ざっているだけでファイル全体が読めなくなるため使わない

`-x` は半角カナの全角化(`ﾃﾞｰﾀ` が `データ` になる)を抑止するために必要

`euc-cat` は `nkf -E -w -x` を使い、nkf が無い環境では `iconv -f EUC-JP-MS` にフォールバックする(波ダッシュが全角チルダになる点だけ妥協する)
...
ただし luit は NEC 特殊文字・IBM 拡張文字を正しく変換できない(`①` が `ⴡ` に、`№` が `ⵢ` になる)。luit が ISO 2022 で登録された文字集合(JIS X 0208 / JIS X 0212)しか持たないためで、波ダッシュと半角カナは正しく扱える。これらの文字を含むファイルを閲覧するだけなら `euc-cat` や `lv` を使う
